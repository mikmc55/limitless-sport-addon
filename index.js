...
app.get('/:userConf/catalog/:type/:id.json', async function (req, res) {
  let {userConf,type,id} = req.params
  let catalogsObj, userConfiguration
  ...
  if(catalogsObj && catalogsObj.id && catalogsObj.id.includes("+")){
    catalogsObj.id = catalogsObj.id.replace(/\+/g,' ')
  }
  ...
  try {
    ...
    if (myCache.has(`catalog-${userConf}-${type}-${id}`)) {
      respond(res,myCache.get(`catalog-${userConf}-${type}-${id}`))
    }else{
      metas = await getCatalog(userConf,type,id)
      if(metas.length > 0){
        myCache.set(`catalog-${userConf}-${type}-${id}`,{metas: metas})
      }
